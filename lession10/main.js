...
function rating(pro) {
  for (let i = 0; i < pro.length; i++) {
    if (pro[i].rating.rate >= 4) {
      console.log(pro[i].title);
    }
  }
}

// bai 3 In ra danh sách các sản phẩm thuộc category: "women's clothing",

function listproduct(pro) {
  for (let i = 0; i < pro.length; i++) {
    if (pro[i].category == "women's clothing") {
      console.log(pro[i].title);
    }
  }
}
...
// bai4 In ra danh sách các category
ex: categories = ["men's clothing", "women's clothing", "jewelery"];
function categ(pro) {
  let newCTG = [];
  for (let i = 0; i < pro.length; i++) {
    if (!newCTG.includes(pro[i].category)) {
      newCTG.push(pro[i].category);
    }
    console.log(newCTG);
  }
}
